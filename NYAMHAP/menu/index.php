...
if ($status === 'create_success') {
    echo '<div class="notice notice-success">Menu baru berhasil ditambahkan! ID: ' . htmlspecialchars($new_id) . '</div>';
} elseif ($status === 'edit_success') {
    echo '<div class="notice notice-success">Data Menu berhasil diperbarui.</div>';
} elseif ($status === 'delete_success') {
    echo '<div class="notice notice-success">Menu berhasil dihapus.</div>';
...
<div class="actions-top">
    <div class="left">
        <?php if ($is_allowed) : ?>
            <a href="create.php" class="btn btn-primary">+ Tambah Menu Baru</a>
        <?php else : ?>
            <span style="color: grey;">* Hanya dapat dikelola oleh Founder dan Staff Produksi.</span>
        <?php endif; ?>
    </div>
...
<table border="1" class="table table-bordered" style="width:100%; border-collapse:collapse;">
    ...
    <tbody>
        ...
        $count_res = pg_query($koneksi, "SELECT COUNT(*) AS cnt FROM menu");
        $total_rows = 0;
        if ($count_res) {
        ...
        if ($per_page_all) {
            ...
            $query = "SELECT menuid, namamenu, deskripsi, harga FROM menu ORDER BY namamenu ASC";
            ...
            $query = "SELECT menuid, namamenu, deskripsi, harga FROM menu ORDER BY namamenu ASC LIMIT $per_page OFFSET $offset";
            $result = pg_query($koneksi, $query);
        }

        $colspan = $is_allowed ? 5 : 4;

        if ($result && pg_num_rows($result) > 0) {
            while($row = pg_fetch_assoc($result)) {
                $menu_id = htmlspecialchars($row['menuid']);
                $menu_id_url = urlencode($row['menuid']);
                $nama = htmlspecialchars($row['namamenu']);
                ...
                if ($is_allowed) {
                    echo "<td>";
                    echo "<a href='edit.php?id=".htmlspecialchars($menu_id_url)."' class='btn btn-warning btn-sm'>Edit</a> ";
                    echo "<a href='delete.php?id=".htmlspecialchars($menu_id_url)."' class='btn btn-danger btn-sm' onclick='return confirm(\"Yakin hapus menu ini?\")'>Hapus</a>";
                    echo "</td>";
                }
            ...
            echo "<tr><td colspan='{$colspan}' style='text-align:center'>Belum ada data menu.</td></tr>";
        }
        ?>
    </tbody>
</table>
